...
class Block(nn.Module):
    ...
    (1) DwConv -> LayerNorm -> 1x1 Conv -> GELU -> 1x1 Conv; all in (N, C, H, W)
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        stem = nn.Sequential(
            ...
            Permute([0, 2, 3, 1]),
            torch.nn.LayerNorm(dims[0], eps=1e-6),
            Permute([0, 3, 1, 2])
        )
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                Permute([0, 2, 3, 1]),
                torch.nn.LayerNorm(dims[i], eps=1e-6),
                Permute([0, 3, 1, 2]),
...
def convnext_small_2(pretrained=False,in_22k=False, **kwargs):
    model = ConvNeXt(depths=[3, 3, 9, 3], dims=[128, 256, 512, 1024], **kwargs)
    return model
